...
export const Exclusive = () => {
    ...
    const exclusiveLog = exclusiveData.map(exclusive => (
      <section key={exclusive.id} className=''>
        <div className="bg-[#D9D9D9] overflow-hidden">
        <Link href={`/products/${exclusive.name.split(" ").join("-")}`}>
          <div>
            <Image 
              src={`/shoes/${exclusive.image}`}
              alt={exclusive.name} 
              width={700} 
              height={700}
    ...
    return (
      <section className="">
        <div className="pt-20 md:pt-40 md:mx-7 mx-7">
          <h3 className='text-3xl md:text-5xl w-full'>Exclusive</h3>
